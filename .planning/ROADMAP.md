...
- **Phase 7: Добавить вход через Google OAuth** — Add Google OAuth sign-in flow with secure account linking and role-compatible session bootstrap.
- **Phase 8: В профиль пользователя добавить компонент с ID и кнопкой копировать ID в буфер обмена** — Surface canonical user ID in profile with one-click copy UX.
- **Phase 9: Локализация интерфейса на русский язык** — Introduce Russian localization for core web UI flows with consistent i18n coverage.
...
### Phase 7: Добавить вход через Google OAuth
...
**Goal:** Добавить в профиль пользователя блок с каноническим user ID и кнопкой copy-to-clipboard с явной success/error обратной связью.
**Depends on:** Phase 7
**Requirements:** WEB-UX-01, WEB-UX-02, QA-01
**Plans:** 0 plans
...
### Phase 9: Локализация интерфейса на русский язык

**Goal:** Локализовать основной web UX на русский язык (navigation, auth, dashboard, collections, forms/errors) с fallback на English и без mixed-language regressions.
**Depends on:** Phase 8
**Requirements:** I18N-01..I18N-08, QA-02
**Plans:** 0 plans
...
### Phase 999.3: Добавить pg_trgm для улучшения качества поиска (BACKLOG)
