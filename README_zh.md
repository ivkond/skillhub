...
- **自托管与私有化** — 部署在您自己的基础设施上。将专有技能保留在防火墙后，完全掌控数据主权。一条 `docker compose` 命令即可在本地运行。
...
默认执行 `docker compose -f compose.dev.yml up` 时，后端以 `local` profile 启动。
...
├── Makefile              # 常用任务
├── compose.dev.yml       # 本地开发栈（build.context）
├── compose.prod.yml      # 生产/预发布栈（镜像名来自 .env）
└── docker-compose.yml    # 仅依赖服务（scanner + postgres + redis + minio）
```
