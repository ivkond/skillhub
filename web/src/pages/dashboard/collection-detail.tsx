...
export function CollectionDetailPage() {
  ...
  const canReorderSkills = !!data && (isOwner || isAdminEquivalent || isContributor)
          ...
          <section className="space-y-3">
            ...
            {members.length === 0 ? (
              <Card>
                ...
                <CardContent>
                  <Link to="/dashboard/skills" className={buttonVariants()}>
                    {t('collections.emptySkillsCta')}
                  </Link>
                </CardContent>
              </Card>
            ) : (
            ...
            <section className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <AddCollectionContributorDialog collectionId={collectionId}>
                  <Button type="button" variant="outline">{t('collections.contributorsAdd')}</Button>
                  ...
                  {contributorsQuery.isLoading ? (
                    ...
                    <ul className="space-y-2">
                      {contributorsQuery.data?.map((contributor, index) => (
                        ...
                        >
                          <span className="font-mono text-sm">{contributor.userId ?? '-'}</span>
                          <Button
                            type="button"
                            variant="destructive"
                            size="sm"
