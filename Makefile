...
build-backend: ## 构建后端
	cd server && ./gradlew clean build -x test

test-backend: ## 运行后端单元测试
	cd server && JDK_JAVA_OPTIONS="$(BACKEND_TEST_JAVA_OPTIONS)" ./gradlew test

build-backend-app: ## 构建 skillhub-app 及其依赖模块
	cd server && ./gradlew :skillhub-app:clean :skillhub-app:bootJar -x test

test-backend-app: ## 运行 skillhub-app 及其依赖模块测试
	cd server && JDK_JAVA_OPTIONS="$(BACKEND_TEST_JAVA_OPTIONS)" ./gradlew :skillhub-app:test
...
db-reset: ## 重置数据库
	...
	cd server && ./gradlew :skillhub-app:flywayMigrate
...
staging: ## 构建并启动 staging 环境，运行 smoke test（混合模式：后端镜像 + 前端静态文件）
	...
	cd server && ./gradlew build -x test --no-daemon --quiet
